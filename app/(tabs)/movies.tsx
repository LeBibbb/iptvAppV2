...
export default function MoviesScreen() {
  const { state } = useData();
  const { vodStreams } = state;

  const [page, setPage] = useState(1);
  const dataPaginated = vodStreams.slice(0, page * PAGE_SIZE);

  const getImageUri = (item: any) => item.stream_icon || "https://via.placeholder.com/120x180.png?text=Film";
  ...
  return (
    <View style={styles.container}>
      ...
      />
      {vodStreams.length > page * PAGE_SIZE && (
